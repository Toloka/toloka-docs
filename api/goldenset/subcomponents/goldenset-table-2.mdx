**string \| mandatory**<br/><br/>Values that are checked in the condition.
...
- `golden_set_incorrect_answers_rate` — The percentage of incorrect responses in training and control tasks (from 0 to 100).<br/><br/>Multiple conditions with the same `key` value are combined using the AND operator.