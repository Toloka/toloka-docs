---
title: "Assign skill to Toloker | Toloka-Kit"
date: "2023-02-07T15:40:37.087559"
author: ["Toloka Team"]
coverId: F_bIcOYF9yjrxkPZrXc3
tags: ["engineering"]
ogDescription: ""
docsMenu: "tkitMenu"
---

import Import from './subcomponents/recipes/import.mdx'
import Instantiate from './subcomponents/recipes/instantiate.mdx'
import SetUserSkill from './subcomponents/set_user_skill.mdx'

# Assign skill to Toloker

_Assign an existing skill to Tolokers an set its level._

## Steps to follow

<Import/>

<Instantiate/>

### 3. Assign skill to Toloker {#step-three}

<CodeCopy
  isSmall
  theme="light"
  code={
    <HighlightChildren language="python">
      from decimal import Decimal
      <br/><br/>user_skill = toloka_client.<Term term="set_user_skill"><SetUserSkill/></Term>(
      <br/>    user_id='1ad097faba0eff85a04fe30bc04d53db',
      <br/>    skill_id='11051', value=Decimal(50)
      <br/>)
    </HighlightChildren>
  }
  textToCopy={`from decimal import Decimal
user_skill = toloka_client.set_user_skill(
    user_id='1ad097faba0eff85a04fe30bc04d53db',
    skill_id='11051', value=Decimal(50)
)`}
/>

### 4. Print created skill information {#step-four}

The `set_user_skill()` request will return the [UserSkill](/docs-new/toloka-kit/reference/toloka.client.user_skill.UserSkill) class object. You can use its attributes to print the information you need.

```python
print(user_skill.id, user_skill.skill_id, user_skill.user_id, user_skill.value)
```

You should get an output with the ID of the skill-Toloker pair (this allows [removing skills from Tolokers](/docs-new/toloka-kit/recipes/delete-user-skill) later), assigned skill ID, Toloker ID, and the skill level the Toloker received which looks like this.

```bash
54132692 12648 1ad097faba0eff85a04fe30bc04d53db 50
```

## Complete code: Create skill {#complete-code}

```python
import toloka.client as toloka
from decimal import Decimal

toloka_client = toloka.TolokaClient('AQC2AGAJgyNSA8CtpdO9MWy_QEB6s6kDjHUoElE', 'PRODUCTION')

user_skill = toloka_client.set_user_skill(
                user_id='1ad097faba0eff85a04fe30bc04d53db',
                skill_id='12648', value=Decimal(50)
)
print(user_skill.id, user_skill.skill_id, user_skill.user_id, user_skill.value)
```

<Note mode="tip" title="List of classes and methods used in this recipe">

- _[TolokaClient](/docs-new/toloka-kit/reference/toloka.client.TolokaClient) class_
- _[set_user_skill()](/docs-new/toloka-kit/reference/toloka.client.TolokaClient.set_user_skill) method_

</Note>

## See also {#see-also}

- [How does Toloka work?](https://toloka.ai/en/docs/guide/concepts/overview)
- [Sample project recipe](/docs-new/toloka-kit/recipes/learn-basics)
- [More usage examples](/docs-new/toloka-kit/recipes/use-cases)
- [Toloka-Kit: UserSkill class](/docs-new/toloka-kit/reference/toloka.client.user_skill.UserSkill)
- [Toloka API: Set skill](https://toloka.ai/docs/api/api-reference/#put-/user-skills)
