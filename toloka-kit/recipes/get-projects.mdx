...
<CodeCopy
  ...
  code={
    <HighlightChildren language="python">
      for project in toloka_client.<Term term="get_projects"><GetProjects/></Term>(
      <br/>                  status='ACTIVE',
      <br/>                  created_gte=datetime(2022, 10, 1)):
      <br/>    print(project.id, project.public_name, project.created.date())
    </HighlightChildren>
  }
  textToCopy={`for project in toloka_client.get_projects(
                  status='ACTIVE',
...
- _[get_projects()](/docs-new/toloka-kit/reference/toloka.client.TolokaClient.get_projects) method_
...
- [Get project details](/docs-new/toloka-kit/recipes/get-project-by-id)
