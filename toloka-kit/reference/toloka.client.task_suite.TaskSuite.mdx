...
date: "2023-02-07T15:41:38.298474"
author: ["Toloka Team"]
...
`tasks`|**Optional\[List\[[BaseTask](/docs-new/toloka-kit/reference/toloka.client.task.BaseTask)\]\]**|<p>The tasks.</p>
...
`mixed`|**Optional\[bool\]**|<p>[The way of grouping tasks](/docs-new/guide/distribute-tasks-by-pages) to create the task suite.</p> <ul> <li>`True` — The tasks are mixed automatically using the smart mixing approach.</li> <li>`False` — The tasks are grouped manually.</li> </ul> <p></p><p>Default value: `False`.</p>
...
`automerged`|**Optional\[bool\]**|<ul> <li>`True` — The task suite was created after [merging tasks](/docs-new/api/tasks#task-merge).</li> <li>`False` — There are no merged tasks in the task suite.</li> </ul>
`created`|**Optional\[datetime\]**|<p>The UTC date and time when the task suite was created. Read-only field.</p>
## Methods Summary

| Method | Description |
| :------| :-----------|
[add_base_task](/docs-new/toloka-kit/reference/toloka.client.task_suite.TaskSuite.add_base_task)| None
