...
date: "2023-02-07T15:42:11.190557"
author: ["Toloka Team"]
...
# AppBatch
`toloka.client.app.AppBatch` | [Source code](https://github.com/Toloka/toloka-kit/blob/v1.1.3/src/client/app/__init__.py#L199)
...
`status`|**Optional\[[Status](/docs-new/toloka-kit/reference/toloka.client.app.AppBatch.Status)\]**|<p>The batch status:</p> <ul> <li>`NEW` — The processing of the batch items is not started.</li> <li>`PROCESSING` — Batch items are being processed by Tolokers.</li> <li>`COMPLETED` — Annotation of all batch items is completed.</li> <li>`ERROR` — An error occurred during processing.</li> <li>`CANCELLED` — Batch processing cancelled.</li> <li>`ARCHIVE` — The batch is archived.</li> <li>`NO_MONEY` — There are not enough money for processing.</li> </ul>
...
`last_items_import`|**Optional\[[AppItemImport](/docs-new/toloka-kit/reference/toloka.client.app.AppItemImport)\]**|<p>Meta-information on asynchronous loading operation (possible via UI).</p>
