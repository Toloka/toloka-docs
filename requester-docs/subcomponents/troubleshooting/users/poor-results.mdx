...
In addition to the training, be sure to set up [quality control rules](/docs/guide/control) in your main pools. This lets you control the quality throughout the task completion process. If the task requires that Tolokers send free-format task responses or data files, use [manual review](/docs/guide/offline-accept) to pay for them only after reviewing the responses.
