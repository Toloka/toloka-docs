...
A task suite that a [Toloker](#toloker) decides to complete. An assignment can have one the following statuses: **accepted**, **active**, **expired**, **rejected**, **skipped**, or **submitted**. Refer to the [Toloka API documentation](/docs/api/result#query-params) for more information on each status.
