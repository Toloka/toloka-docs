<Cut title="How are the correct responses to control questions counted?">

The [Control tasks](/docs/guide/goldenset) rule starts working after the Toloker completes the number of control tasks you specified. If your pool contains both [training](/docs/glossary#training-task) and control tasks, you can take into account the responses in both of them (the **Number of responses** parameter) or only in control tasks (the **Number of control responses** parameter).
