...
This helper is similar to a series of [If...Then...Else](/docs/template-builder/reference/helper.if) logical expressions, so it is useful if there are more than two conditions for sequential verification. If you need to check one or two conditions, use the [helper.if](/docs/template-builder/reference/helper.if) component.
...
<Button themeColor="light" view="default" size="l" type="link" url="/docs/guide/troubleshooting/support?form-source=template-builder-reference-helper-switch">Contact support</Button>
