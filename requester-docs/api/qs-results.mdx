...
date: "2023-02-17T14:32:31.057207"
author: ["Toloka Team"]
...
In the `pool_id` query parameter, replace `<pool id>` with the ID of the pool you want to get responses for (the ID received in response to the [add pool request](/docs/api/qs-placement#pool)).
...
- [HTTP methods used for processing Toloker responses](/docs/api/get-response)
- [HTTP methods used for processing files in Toloker responses](/docs/api/attachments)
- [Response aggregation](/docs/api/aggregated-solutions)
- [Using libraries](/docs/api/libraries)
