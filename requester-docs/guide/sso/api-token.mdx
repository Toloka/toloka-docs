...
After you login to the platform with [single sign-on](/docs/guide/sso/authentication) (SSO) authentication method, you can generate new tokens. You can use them for API requests or for the integration applications. The previously generated tokens will continue to work, but will be deprecated with time.
...
1. Go to **Profile → Integrations** or follow [this link](https://platform.toloka.ai/requester/profile/integration). Click **<img src="https://tlkfrontprod.azureedge.net/portal-production/static/uploaded/images/1675425166901/1675425166901_original_svg.svg" alt="Plus sign" /> Add token**:
...
Now you can use the created API token to send other API requests: get the list of the existing [projects](/docs/api/get-prj-list), [pools](/docs/api/get-pool-list), [upload tasks](/docs/api/upload-tasks), and other requests.
