To get a file with Tolokers' responses, click the **Download results** button on the [pool](/docs/glossary#pool) page.
...
- `OUTPUT:<name of the output data field>` — The Tolokers' responses. For [training tasks](/docs/glossary#training-task), the field includes first attempted answers.
...
- Information about the task completion (the same for all tasks on the page):
    ...
    - `ASSIGNMENT:link` — Link for viewing the [task suite](/docs/glossary#task-suite).
...
<Image width="700px" withBackground imageId="1675426532723" alt="" />