...
Use [decomposition](/docs/guide/solution-architecture). Simpler tasks cost less. If a task is complex, you won't get good results immediately. Instead, you'll have to resend tasks to the pool so that other Tolokers can redo them.

## Use dynamic overlap {#din-price}

[Dynamic overlap](/docs/guide/dynamic-overlap) (incremental relabeling, IRL) lets you change overlap depending on:
...
You can set up a [random majority vote check](/docs/guide/selective-mvote) for the general tasks. This helps you save money and speed up pool completion. You can set the period during which tasks with increased overlap will be displayed to the Toloker, depending on the number of completed tasks.
...
[Dynamic pricing](/docs/guide/dynamic-pricing) lets you increase the price per task suite for Tolokers with a higher skill level. This way you can encourage Tolokers to do a better job.
...
Use the [Smart mixing](/docs/guide/distribute-tasks-by-pages#smart-mixing) and **Assign partial page** options. This way you can save money on paying for incomplete task suites.
...
[Learn more](/docs/api/create-pool)
...
[Learn more](/docs/api/create-task)
...
<Button themeColor="light" view="default" size="l" type="link" url="/docs/guide/troubleshooting/support?form-source=guide-save-money">Contact support</Button>
