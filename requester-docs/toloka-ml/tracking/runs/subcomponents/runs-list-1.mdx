To add a run to Toloka ML platform, set the `ModelInfo`, `Metrics`, `Metric`, and `DatasetInfo` objects. Next, create metrics and a run object and pass your [OAuth token](/docs/toloka-ml/reference/#token) and the run object to the client.
