---
title: "Add a button — Tutorials | Toloka documentation"
date: "2023-02-14T11:23:08.368162"
...
<Cut title="What does it look like?">

Before:

<Image width="700px" withBackground imageId="1675426298516" alt="" />

After:

<Image width="700px" withBackground imageId="1675426243441" alt="" />

When the Toloker clicks the new button:

<Image width="700px" withBackground imageId="1675426237603" alt="" />
...
exports.Task = extend(TolokaHandlebarsTask, function (options) {
    ...
    initFastFileSelector: function() {
        var <Formula withoutContainer={true} content="$el = $"/>(this.getDOMElement()),
            sources = [],
            type = '',
            audioRecorder = <Formula withoutContainer={true} content="$el.find('.audioRecorder');

        $"/>el.find('.field_file-img__upload_camera').each(function (i,el) {
            <Formula withoutContainer={true} content="$(el).on('click',function () {
                sources = ['CAMERA'];
                type = 'IMAGE';
            })
        });

        $"/>el.find(".field_file-img__label").prepend(<Formula withoutContainer={true} content="$("<span/>", {class: "field_file-img__upload gallery"}));
...
<Button themeColor="light" view="default" size="l" type="link" url="/docs/guide/troubleshooting/support?form-source=tutorials-advanced-features-button">Contact support</Button>
