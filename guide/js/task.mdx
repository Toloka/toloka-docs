...
The `Task` base class is responsible for the [task interface](/docs-new/glossary#task-interface). This class is available in the global variable `window.TolokaTask`.
...
The [task UI](/docs-new/guide/spec) constructor.

Parameters:

- `options.task` — The [Task](/docs-new/guide/spec-advanced#obj-task) model.
...
Returns the [Solution](/docs-new/guide/spec-advanced#obj-solution) task responses.
...
Returns the [Task](/docs-new/guide/spec-advanced#obj-task) task model.
...
- `errors` — Validation errors in the Toloker's response [SolutionValidationError](/docs-new/guide/spec-advanced#obj-solutionvalidationerror).
...
- `solution` — The Toloker's response to the task ([Solution](/docs-new/guide/spec-advanced#obj-solution)).
...
Template engine for the task. In the task's [HTML interface](/docs-new/guide/spec), it replaces occurrences such as `${fieldX}` with the corresponding value and the `fieldX` key from the `data` parameter. Returns the task's HTML interface as a string. Parameter:

- `data` — Object with data to insert in the template.

## validate(solution) {#validate}

Validates responses according to output data parameters. Returns [SolutionValidationError](/docs-new/guide/spec-advanced#obj-solutionvalidationerror) if the responses are invalid, otherwise `null`. Parameter:
