...
This class manages the task progress, processes the [task suite](/docs-new/glossary#task-suite) commands for sending responses, skipping or pausing tasks, and more. It also creates an instance of the [TaskSuite](/docs-new/guide/tasksuite) class.
...
Calls [TaskSuite.destroy()](/docs-new/guide/tasksuite#destroy), removes all template elements from the DOM, closes the messaging channel, stops hotkey polling and location detection, calls `onDestroy`.

## initHotkeys() {#inithotkeys}

Hotkey handler initializer:

- Resets all hotkeys ([hotkeys.reset](/docs-new/guide/services#reset)).
...
Returns a link to a [TaskSuite](/docs-new/guide/tasksuite) instance.
...
Collects answers to all tasks ([TolokaTaskSuite.getSolutions()](/docs-new/guide/tasksuite#getSolutions)), validates them ([TolokaTaskSuite.validate()](/docs-new/guide/tasksuite#validate)). If validation is successful, calls `strategy`, otherwise sends an `assignment:validation:fail` request and an object with a list of errors returned by the validator. Parameter:
