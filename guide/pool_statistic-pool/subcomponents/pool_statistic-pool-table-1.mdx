...
<Formula withoutContainer={true} content="$Assign_{left}$"/> is the number of task suite remaining uncompleted or those completed with insufficient [overlap](/docs-new/glossary#overlap).
