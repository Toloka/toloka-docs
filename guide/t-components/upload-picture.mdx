...
The task interface configuration guide describes the features of the HTML/JS/CSS editor. You can also try creating a button for uploading images in [Template Builder](/docs-new/template-builder/reference/field.media-file).

</Note>

To add an image upload button <img src="https://tlk-infra-front.azureedge.net/portal-static/uploaded/images/1675425180691/1675425180691_original_svg.svg" alt="" /> to the task, use the `{{field type= "file-img" name="<output field name> "sources="<sources parameter>" fileType="<fileType parameter>"}}`. After the image is uploaded the Toloker will see an icon for image preview. Example:
