...
The task interface configuration guide describes the features of the HTML/JS/CSS editor. You can also try creating a button with click validation in [Template Builder](/docs-new/template-builder/operations/internet-search#action.open-link).
...
- Checking if the Toloker opened a link. The [output data](/docs-new/glossary#input-output-data) will contain a field with values `true` (the Toloker opened the link) or `false` (the Toloker didn't open the link).
...
To add the button with the link to the [task interface](/docs-new/glossary#task-interface), use the `{{field type="button-clicked" name="output field name>"}}` component. Example:
