...
Captchas and the [Fast responses](/docs/guide/quick-answers) rule provide a high level of protection from robots.

To show a captcha to Tolokers, set captcha frequency in the [pool](/docs/glossary#pool) settings.
...
**Field**  | **Overview**
----- | -----
**Recent captchas to use** | The number of times the Toloker was shown a captcha recently.<br/><br/>If this field is not filled in, the calculation includes only captcha entries from the pool to which the rule is applied.<br/><br/>If the field is filled in, the corresponding number of captcha entries is used. The rule takes into account captcha entries from both this pool and other pools where this field is filled in.<br/><br/>[Learn more](/docs/guide/remember-values) about how this field works.
**If** | <CaptchaTable1/>
**then** | <CaptchaTable2/>
...
- [Toloka API: Using captcha](/docs/api/captcha)
- [Toloka-Kit: Captcha class](/docs/toloka-kit/reference/toloka.client.collectors.Captcha)
