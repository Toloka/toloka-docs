...
We take all Tolokers' responses and, for example, option <Formula withoutContainer={true} content="$z[x]$"/> and calculate the probability that Tolokers will select this response, provided that the correct response is <Formula withoutContainer={true} content="$z[x] estimate,$"/>:
...
1. [Create a pool](/docs-new/guide/pool-main) and fill in the [parameters](/docs-new/guide/pool_poolparams).
...
1. Upload tasks via [smart mixing](/docs-new/guide/distribute-tasks-by-pages#smart-mixing).

1. After the pool is completed, start [Aggregation of results by skill](/docs-new/guide/result-aggregation#aggr-by-skill).

<Note mode="note" title="Note">

**Approximate budget** in the [pool statistics](/docs-new/guide/pool_statistic-pool) is calculated based on the minimal overlap.

</Note>

<Cut title="Example">

Configuring dynamic overlap for [simple classification](/docs-new/tutorials/image-classification).

<Image withBackground imageId="1675426723898" alt="" />
...
- Don't use the [Recompletion of assignments from banned users](/docs-new/guide/restore-task-overlap) quality control rule together with dynamic overlap. This increases the maximum overlap value.
...
- Upload tasks via **smart mixing**. Otherwise, the tasks are assigned with minimum overlap.
    ...
    This is necessary for overlap to increase because [control](/docs-new/glossary#control-task) and [training](/docs-new/glossary#training-task) tasks uploaded via **smart mixing** have infinite overlap.

- Use a skill calculated as the [percentage of correct responses to control tasks](/docs-new/guide/goldenset). This will give you the most accurate results.
...
- [Adding tasks](/docs-new/guide/pool) in the pool.
...
- [Aggregation of results](/docs-new/guide/result-aggregation)

## For developers {#for-developers}

- [Toloka-Kit: Dynamic overlap setting](/docs-new/toloka-kit/reference/toloka.client.pool.dynamic_overlap_config.DynamicOverlapConfig)
