...
If tasks were issued with an [overlap](/docs-new/glossary#overlap) of 2 or higher, run aggregation of results. Toloka will process all Tolokers' responses for the task and issue the resulting response and its confidence level.
...
If you run the [pool](/docs-new/glossary#pool) with the manual review, make sure that all responses are [accepted](/docs-new/guide/accept).
...
- The Dawid-Skene aggregation model works with [control](/docs-new/glossary#control-task) and [training](/docs-new/glossary#training-task) tasks as well as with general tasks. There is a possibility that the `OUTPUT:result` field for the control task in the TSV file won't match the actual response to this task (`GOLDEN:result`).
...
If you want to learn how the Dawid-Skene method is implemented in Toloka, check out the [Crowd Kit documentation](/docs-new/crowd-kit/reference/crowdkit.aggregation.classification.dawid_skene.DawidSkene).
...
- [Toloka API: Aggregated responses](/docs-new/api/aggregated-solutions)
- [Toloka-Kit recipe: Aggregate responses in pool](/docs-new/toloka-kit/recipes/aggregate-responses)
