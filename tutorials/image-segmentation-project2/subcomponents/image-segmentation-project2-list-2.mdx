...
1. Edit the **CSS** block:
    ...
    1. Set up the area selection tools. This template uses [Editor for image area selection](/docs-new/guide/t-components/image-annotation). It features rectangle and polygon tools (by default).
    ...
    1. **(optional)** You can ask Tolokers to enter an annotation for the selected area or select one from a list. To do this, add an interface element in the **JS** block. For example, a text field or drop-down list.
        ...
        Learn more about [annotation](/docs-new/guide/t-components/image-annotation#annotation).
