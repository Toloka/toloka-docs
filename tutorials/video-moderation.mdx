...
- Upload your video clips to a storage from which you can get file links. For example, your own server, a video hosting, or a [cloud storage](https://toloka.ai/en/docs/guide/concepts/cloud-storage#cloud).
...
- Make sure that the video clips look the same in the desktop and mobile versions of Toloka. A video clip may not play or play incorrectly in the mobile version. In this case, when creating a pool, you will need to restrict access to the pool for the Tolokers with mobile devices.
    ...
    Learn more about [configuring tasks for mobile devices](https://toloka.ai/en/docs/guide/concepts/mobile).
...
1. In the **Task interface** section, set up what your tasks will look like. This preset has a task template with validation, keyboard shortcuts, and task layout pre-configured.
    ...
    1. <Cut title="If you use videos from YouTube">
        ...
        1. Go to the **Code editor** tab. In the **Config** section, replace the `view.video` value with `view.iframe`.
            ...
            To learn about other properties of the **Config** section, their possible values and the impact on the task interface, see the [Template Builder Help](/docs-new/template-builder/templates/video-moderation).
...
- [Template Builder: Video analysis](/docs-new/template-builder/templates/video-moderation)
