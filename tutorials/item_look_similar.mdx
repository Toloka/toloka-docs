...
In this [project](/docs-new/glossary#project), Tolokers will compare shoes from the original image (shown in project 1) with shoes found in project 2.

<Note mode="warning" title="Warning">

Since the **Assignment review** option is enabled in the [pool](/docs-new/glossary#pool) settings for project 2, you should complete all the steps and check the Tolokers' responses within the time limit set in the **Review period** field.
...
1. In **Instructions for Tolokers**, enter the [instructions](/docs-new/glossary#instructions).
...
Learn more about working with the project in the [Project](/docs-new/guide/project) section.
...
1. (optional) In the **Speed/quality balance** section, specify the desired quality level. Improving quality may reduce the speed of task completion because the pool will be available to fewer Tolokers. Learn more in the [Speed/quality balance](/docs-new/guide/adjust) section.
...
1. <TutorialsQualityControl/>
    ...
    1. Set a rule for [control tasks](/docs-new/glossary#control-task): If the **number of responses** to control questions **is greater than or equal to 3** and the **percentage of correct responses** to control questions **is less than 60**, then **restrict the Toloker's access** to the **project for 10 days**. Specify the **Control task** as a reason.
    ...
    Learn more in [Quality control](/docs-new/guide/control).
...
1. <TutorialsUploadTasks/>
    ...
    1. Prepare a [file](/docs-new/glossary#tsv) with tasks.
...
1. <TolokaRequesterSourceAddControlTasks/>
    ...
    1. Click **Are these shoes similar to each other?** to exit [task markup](/docs-new/glossary#task-markup) mode.

        <Note mode="note" title="Note">

        In small pools, [control tasks](/docs-new/guide/troubleshooting/troubleshooting#how-many-control-tasks) should be 5–10% of all tasks. Include different versions of correct responses in equal amounts. See the distribution of responses on the **Edit tasks** page, **Control tasks** tab.
...
1. Choose the **Dawid-Skene aggregation model**. Learn more about [Result aggregation based on the Dawid-Skene model](/docs-new/guide/result-aggregation#dawid-skene).
...
To review the [submitted assignments](/docs-new/glossary#completed-tasks):
