---
title: "Reviewing assignments automatically | Template Builder"
date: "2023-02-10T09:15:29.211993"
author: ["Toloka Team"]
...
- What can be reviewed:
    - Output data in [fields.*](/docs-new/template-builder/reference/fields);
    - Input data in [views.*](/docs-new/template-builder/reference/views) and [layouts.*](/docs-new/template-builder/reference/layouts).

- Where to specify conditions:
    - In the `validation` property of the parent component.
    - In the `condition` property of [plugin.trigger](/docs-new/template-builder/reference/plugin.trigger).

- A link to the reviewed data is specified in the `data` property.
- Some components don't have the `data` property, and you can only access them from the `validation` property of the parent component (for example, [condition.played](/docs-new/template-builder/reference/condition.played) and [condition.played-fully](/docs-new/template-builder/reference/condition.played-fully)).

## Making a field required {#condition-required}

To check that the data is filled in, add the [condition.required](/docs-new/template-builder/reference/condition.required) component.

In this example, a Toloker must fill in the text field, otherwise the “Submit” button won't work. The [condition.empty](/docs-new/template-builder/reference/condition.empty) component works the opposite way: it checks if the data is empty and returns `false` if the data has a value.
...
To check that the preset value and output value are equal, use the component [condition.equals](/docs-new/template-builder/reference/condition.equals).
...
To return the values that don't meet the specified conditions, list them in the `condition` property of the [condition.not](/docs-new/template-builder/reference/condition.not) component.
...
To check that an array in `data` is a subarray for `parent`, add the component [condition.sub-array](/docs-new/template-builder/reference/condition.sub-array).
...
To check that multiple conditions are met, list them in the `condition` property of the [condition.all](/docs-new/template-builder/reference/condition.all) component.
...
To check that at least one of the conditions is met, list the conditions in the `condition` property of the [condition.any](/docs-new/template-builder/reference/condition.any) component.
...
Some conditions work only with certain [components](https://toloka.ai/en/docs/glossary#component). For example, [condition.link-opened](/docs-new/template-builder/reference/condition.link-opened) works only with `view.link`.
...
In the [list of conditions](/docs-new/template-builder/reference/conditions), you can see all the available components with the name `conditions.*`. If you can't find the desired component in the list or want to use a preset [configuration](https://toloka.ai/en/docs/glossary#configuration), define a review in the [JSON Schema](https://json-schema.org/learn/getting-started-step-by-step.html) format using [condition.schema](/docs-new/template-builder/reference/condition.schema).
