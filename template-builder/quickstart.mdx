---
title: "Quick start | Template Builder"
date: "2023-02-09T18:49:31.641166"
author: ["Toloka Team"]
...
1. **Sent data**: sample JSON output. This panel appears when you click **Send** in the preview. To copy the output data, click <img src="https://tlk-infra-front.azureedge.net/portal-static/uploaded/images/1675425153599/1675425153599_original_svg.svg" alt="" />. To hide this panel, click **Close Panel** at the bottom.

To hide or display the first three panels, use the <img src="https://tlk-infra-front.azureedge.net/portal-static/uploaded/images/1675425156149/1675425156149_original_svg.svg" alt="" /><img src="https://tlk-infra-front.azureedge.net/portal-static/uploaded/images/1675425154614/1675425154614_original_svg.svg" alt="" /><img src="https://tlk-infra-front.azureedge.net/portal-static/uploaded/images/1675425155637/1675425155637_original_svg.svg" alt="" /> buttons in the upper-left corner.

<Note mode="note" title="Note">

For some Toloka presets, the visual editor is available. Refer to the [Visual editor](/docs-new/template-builder/visual-editor) section to learn more about how to edit the task interface using it.

</Note>

If you opened Template Builder in Toloka, click <img src="https://tlk-infra-front.azureedge.net/portal-static/uploaded/images/1675425155125/1675425155125_original_svg.svg" alt="" /> in the upper-right corner to switch to full-screen mode. If you opened the Template Builder sandbox, it is already displayed full-screen.
...
- `view`: The component to be displayed, such as an image or list of nested components.
    ...
    In the `type` property, specify the name of the component, then list the other properties. Each component has its own set of properties. See the hints in the editor and the component descriptions in the [Component reference guide](/docs-new/template-builder/reference/).

- `plugins`: An array of added [plugins](/docs-new/template-builder/reference/plugins). These are special components that provide additional functionality, such as shortcuts.

- `vars`: Variables with JSON code that can be [reused](/docs-new/template-builder/best-practices/reuse) anywhere in the configuration using the $ref structure as follows:
...
1. The configuration editor will display an empty code template with two blocks: `view` and `plugins`.
    ...
    The `view` block has the [view.list](/docs-new/template-builder/reference/view.list) component to display data in a list. We'll use it to create the interface we need.
...
1. The next interface element is the [field.button-radio-group](/docs-new/template-builder/reference/field.button-radio-group) component that adds a group of buttons. Its `label` property contains the text that will be displayed above the answer options. Change its value to `"What is the cat's mood?"`.
...
1. The next property after the group of buttons is `data`. It determines what the output data will look like ([data.output](/docs-new/template-builder/operations/work-with-data)). The output data is the values that take the responses to the task questions.

1. The [plugin.toloka](/docs-new/template-builder/reference/plugin.toloka) component at the end of the code specifies the additional task settings. Change the value of the `taskWidth` property (the width of the task block) to 500.
...
Use code snippets to see what property values you can use. In autocomplete, snippets are labeled with the <img alt="" src="https://tlk-infra-front.azureedge.net/portal-static/uploaded/images/1675425319510/1675425319510_original_png.png" /> icon.
...
Learn more about shortcuts in [Configure keyboard shortcuts](/docs-new/template-builder/best-practices/hotkeys).

Shortcuts are not part of visual components. They are added via a separate plugin, [plugin.hotkeys](/docs-new/template-builder/reference/plugin.hotkeys). Use this plugin to assign keys to actions.
...
To enable this, add the `validation` property with the [condition.required](/docs-new/template-builder/reference/condition.required) component to the component's code. In the examples above, this code has already been added.
...
Learn more about automated checks in [Reviewing assignments automatically](/docs-new/template-builder/best-practices/conditions).
...
- See [instructions](/docs-new/template-builder/operations/all) for popular tasks.
- Check the [list of available components](/docs-new/template-builder/reference/).
