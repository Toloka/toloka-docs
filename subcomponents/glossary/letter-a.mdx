...
A category of [components](#component) that perform actions, such as open a link, display a message, or play a video. Some components work with data. For example, they can change the value from `true` to `false`. [List of actions](/docs-new/template-builder/reference/actions)
