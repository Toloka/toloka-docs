...
An option that allows you to selectively increase overlap for some tasks. This helps you save money and speed up [pool](#pool) completion. [Learn more](/docs-new/guide/selective-mvote)

##### Side-by-side (SbS) {#sbs}

Side-by-side comparison type of projects. Toloka has several presets to compare objects side-by-side: [images](/docs-new/tutorials/side-by-side), video and audio files.

##### Skill {#skill}

An assessment of the Toloker's quality of task completion on a scale of 0 to 100. The skill can be calculated automatically (for example, based on correct responses to [control tasks](#control-task)), or assigned manually. Skills are used to select Tolokers in a [pool](#pool). [Learn more](/docs-new/guide/nav)
