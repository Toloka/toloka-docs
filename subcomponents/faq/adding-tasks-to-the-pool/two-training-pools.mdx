...
Create the first pool based on the [training pool](https://toloka.ai/en/docs/glossary#training-pool) and the second pool based on the main pool with the pool type set to **Exam**. If a pool contains only control and/or training tasks, the price can be set to zero.
